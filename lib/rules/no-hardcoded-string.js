...
const rule = ESLintUtils.RuleCreator(name => `https://example.com/rule/${name}`)({
  ...
  create(context) {
    ...
    function isTranslateFunctionParameter(node) {
      return node.parent.type === 'CallExpression'
        && (
          node.parent.callee.name === 'instant' || (
            node.parent.callee.property
            && node.parent.callee.property.name === 'instant'
          )
        );
    }
    ...
    function stringContainsText(input) {
    ...
    return {
      ...
      'TemplateLiteral'(node) {
        const hasTextContent = node.quasis.some(quasi => stringContainsText(quasi.value.cooked || ''));
        
        if (hasTextContent && !isFunctionParameter(node) 
            && !isInDecorator(node)
            && !isThrowingError(node)
            && !isConsoleLogging(node)
        ) {
        ...
        if (hasTextContent && isTranslateFunctionParameter(node)) {
          reportIfHardcoded(node);
        }
      },
    };
